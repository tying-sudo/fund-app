...
import { resolveWatchlistDisplayValue } from '@/utils/watchlistSnapshot'
...
const displayNetValue = computed(() => {
  const value = resolveWatchlistDisplayValue({
    estimateValue: props.fund.estimateValue,
    lastValue: props.fund.lastValue,
    realChange: props.fund.realChange,
    realChangeDate: props.fund.realChangeDate,
    officialValue: props.fund.officialValue,
    officialValueDate: props.fund.officialValueDate,
    isCurrentReal: comparisonState.value.isCurrentReal
  })
  if (!value || value === '0' || value === '0.0000' || value === '--') return '--'
  return formatNetValue(value)
})
      ...
      <div class="fund-info">
        ...
        <div class="fund-meta">
          ...
          <button
            ...
            title="在支付宝中查看基金"
            @click.stop="openAlipayFundDetail(fund.code)"
            @touchstart.stop
            @touchend.stop
            @touchmove.stop
          > · {{ fund.type }}</button>
